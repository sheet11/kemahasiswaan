...
if($_POST['halaman'] == 0)
{
	...
	if(empty($nama_file))
	{
		$qr = mysql_query("update tb_surat_keterangan_lulus set nama_mahasiswa = '$_POST[nama_mahasiswa]',
		...
		if($tipe_file == "image/jpeg" || $tipe_file == "image/png" || $tipe_file == "image/jpg")
		{
			if($ukuran_file <= 3000000)
			{
				if(move_uploaded_file($tmp_file, $path))
				{
					...
					if($qr)
					{
						echo "<script>alert('Data berhasil diperbarui.');window.location='02_daftar_surat_penelitian.php?username=$_POST[username]'</script>";
					}
					else
					{
						echo "<script>alert('Mohon maaf, data gagal diperbarui.');window.location='02_daftar_surat_keterangan_lulus.php?username=$_POST[username]'</script>";
