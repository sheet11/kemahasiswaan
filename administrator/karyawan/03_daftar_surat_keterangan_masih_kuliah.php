                ...
                <tr>
                    <td colspan="6">
                        ...
                        <a class="btn btn-info"><i class="fa fa-cloud fa-sm"></i> | Jumlah : <?php
                            ...
                            echo mysqli_num_rows(mysqli_query($kon,"SELECT * FROM tb_surat_keterangan_masih_kuliah")); ?></a>
                        <a class="btn btn-success"><i class="fa fa-spinner fa-sm"></i> | Belum Di Proses : <?php
                            echo mysqli_num_rows(mysqli_query($kon,"SELECT * FROM tb_surat_keterangan_masih_kuliah WHERE status='Belum Dicetak'")); ?></a>
                        <a class="btn btn-danger"><i class="fa fa-spinner fa-sm"></i> | Sudah Di Proses : <?php
                            echo mysqli_num_rows(mysqli_query($kon,"SELECT * FROM tb_surat_keterangan_masih_kuliah WHERE status='Sudah Dicetak'")); ?></a>
                    </td>
                    <td colspan="6">
                        <div class="input-group" style="max-width:450px;">
                            <span class="input-group-addon"><i class="fa fa-search"></i></span>
                            <input type="text" id="keyword_mk" class="form-control" placeholder="Cari nama, NIM, jurusan, status..." autocomplete="off">
                            <span class="input-group-btn">
                                <button class="btn btn-default" id="btn_reset_mk" type="button" title="Reset"><i class="fa fa-times"></i></button>
                            </span>
                        </div>
                        <small id="info_mk" class="text-muted"></small>
                        <span id="loading_mk" style="display:none; color:#888; margin-left:8px;"><i class="fa fa-spinner fa-spin"></i></span>
                    </td>
                </tr>
                <tr><td>&nbsp;</td></tr>
            </table>
            ...
            <table style="width:100%;" class="table table-bordered">    
                ...
                <?php 
                    ...
                    function kolomStatus($a) {
                        ...
                        if ($sp == 'Ditolak') {
                            $h = "<span class='label label-danger' style='font-size:12px;'><i class='fa fa-times'></i> Ditolak</span>";
                            if ($catatan) $h .= "<br><small style='color:#dd4b39;'><i class='fa fa-comment-o'></i> <i>$catatan</i></small>";
                            return $h;
                        }
                    ...
                    function kolomAksi($a, $id, $hal='') {
                        $p = $hal ? "&halaman=$hal" : '';
                        $h = '';
                        if ($a['status_persetujuan']=='Disetujui') $h .= "<a href='03_cetak_surat_keterangan_masih_kuliah.php?id_surat_keterangan_masih_kuliah=$id$p' class='btn btn-info btn-xs'><span class='glyphicon glyphicon-print'></span></a> ";
                        $h .= "<a href='03_edit_surat_keterangan_masih_kuliah.php?id_surat_keterangan_masih_kuliah=$id$p' class='btn btn-success btn-xs'><span class='glyphicon glyphicon-pencil'></span></a> ";
                        $h .= "<a href='03_delete_surat_keterangan_masih_kuliah.php?id_surat_keterangan_masih_kuliah=$id$p' onclick='return confirm(\"Hapus {$a['nama_mahasiswa']}?\")' class='btn btn-danger btn-xs'><span class='glyphicon glyphicon-remove'></span></a>";
                        return $h;
                    }
                    ...
                    while ($a = mysqli_fetch_array($query)) {
                        ...
                        $hal = $_GET['halaman'] ?? '';
                        echo "<tr><td>$i</td><td>$a[nama_mahasiswa]</td><td>$a[nim_mahasiswa]</td><td>$a[jurusan]</td><td>$a[tingkat]</td><td>$a[tahun_akademik]</td><td>".kolomStatus($a)."</td><td>".kolomAksi($a,$id,$hal)."</td></tr>";
                        $i++;
                    }
                ?>
                </tbody>
...
(function () {
    ...
    function renderStatus(r) {
        ...
        if (sp==='Disetujui')     return "<span class='label label-success' style='font-size:12px;'><i class='fa fa-check'></i> Disetujui</span>";
        if (sp==='Ditolak') {
            var h="<span class='label label-danger' style='font-size:12px;'><i class='fa fa-times'></i> Ditolak</span>";
            if (c) h+="<br><small style='color:#dd4b39;'><i class='fa fa-comment-o'></i> <i>"+escHtml(c)+"</i></small>";
            return h;
        }
        return "<span class='label label-warning' style='font-size:12px;'><i class='fa fa-clock-o'></i> Menunggu</span>";
    }
    function renderAksi(r) {
        var id=r.id_surat_keterangan_masih_kuliah, nama=escHtml(r.nama_mahasiswa), h='';
        if (r.status_persetujuan==='Disetujui') h+="<a href='03_cetak_surat_keterangan_masih_kuliah.php?id_surat_keterangan_masih_kuliah="+id+"' class='btn btn-info btn-xs'><span class='glyphicon glyphicon-print'></span></a> ";
        h+="<a href='03_edit_surat_keterangan_masih_kuliah.php?id_surat_keterangan_masih_kuliah="+id+"' class='btn btn-success btn-xs'><span class='glyphicon glyphicon-pencil'></span></a> ";
        h+="<a href='03_delete_surat_keterangan_masih_kuliah.php?id_surat_keterangan_masih_kuliah="+id+"' onclick=\"return confirm('Hapus "+nama+"?')\" class='btn btn-danger btn-xs'><span class='glyphicon glyphicon-remove'></span></a>";
        return h;
    }
    function doSearch(kw) {
        ...
        xhr.open('GET','ajax_search_surat.php?tabel=masih_kuliah&keyword='+encodeURIComponent(kw),true);
