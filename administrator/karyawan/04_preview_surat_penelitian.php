...
$a  = mysqli_fetch_array(mysqli_query($kon, "SELECT * FROM tb_surat_penelitian WHERE id_surat_penelitian='$id'"));
if (!$a) { echo "<script>alert('Data tidak ditemukan!'); window.history.back();</script>"; exit; }

$sp       = $a['status_persetujuan'];
$return   = isset($_GET['return']) ? $_GET['return'] : '04_daftar_surat_penelitian.php';
...
<div class="toolbar">
    ...
    <?php if ($level == 'karyawan' || $level == 'administrator'): ?>
        ...
        <?php if ($sp == 'Menunggu' || $sp == 'Telah_Direvisi'): ?>
            <a href="proses_karyawan.php?aksi=setujui&jenis=penelitian&id=<?php echo $id; ?>&return=<?php echo urlencode($return); ?>"
               onclick="return confirm('Setujui dan teruskan ke Wadir?')" class="btn btn-success btn-sm">
                <i class="fa fa-check"></i> Setujui ke Wadir
            </a>
            <a href="proses_karyawan.php?aksi=tolak&jenis=penelitian&id=<?php echo $id; ?>&return=<?php echo urlencode($return); ?>"
               class="btn btn-warning btn-sm">
                <i class="fa fa-times"></i> Tolak
            </a>
        <?php elseif ($sp == 'Disetujui'): ?>
            <a href="04_cetak_surat_penelitian.php?id_surat_penelitian=<?php echo $id; ?>"
...
$status_label = [
    ...
    'Disetujui_Resepsionis' => ['class' => 's-disetujui_resepsionis', 'label' => '<i class="fa fa-check-circle"></i> Status: Disetujui KResepsionis — Menunggu Persetujuan Wadir'],
    ...
    'Perlu_Revisi'          => ['class' => 's-ditolak',               'label' => '<i class="fa fa-edit"></i> Status: Perlu Revisi — Menunggu perbaikan dari mahasiswa' . (!empty($a['catatan_penolakan']) ? ' &mdash; Catatan: ' . htmlspecialchars($a['catatan_penolakan']) : '')],
];
